...
class HepsipayApi
{
    ...
    public static function processPayReaponse($module, $response=[], $use3D=false)
    {
        ...
        $processResult = [
            ...
            'orderCurrencyId' => $currency_id,
            'orderCurrency' => $originalCurrency,
        ...
        try {
            ...
            if($status && !$use3D) {
                ...
                $processResult['orderId'] = $order_id;
            }
            ...
            $message = "ERROR: ".$ex->getMessage(). " (transaction message: $message)";
            $processResult['success'] = false;
            $processResult['message'] = $ex->getMessage();
        } finally {
            if($log_id) {
    ...
    public static function renderAutoSubmitForm($action, $data=[]) {
        $id = '__autoSubmitForm'.time();
        $html = ['<form method="post" action="'.$action.'" id="'.$id.'" name="'.$id.'">'];
        foreach($data as $key=>$val) {
            $html[] = '<input type="hidden" name="'.$key.'" value="'.$val.'">';
        }
        $html[] = '</form>';
        $html[] = '<script type="text/javascript">document.'.$id.'.submit();</script>';
