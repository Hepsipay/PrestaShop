(function($) {
    hepsipayModule = {
        ...
        run: function(config) {
            ...
            $('#hepsipay_payment_form').on('submit', function(){
                var $form = $(this);
                $('#hepsipay_payment_form_submit_btn').prop( "disabled", true );
                $form.find('.form-group').removeClass('has-error');
                $form.find('.help-block').html('');
                $form.find('.general-error').hide();
                $form.find('.processing-payment').show();
                    ...
                    success: function(response) {
                        if(!response.dataValid) {
                            for(var i in response.errors) {
                                var $g = $form.find('input[name="'+i+'"]').closest('.form-group');
                                $g.addClass('has-error');
                                $g.find('.help-block').html(response.errors[i]);
                            }
                        } else if(!response.success) {
                            $form.find('.general-error').show().find('.alert').html(response.message);
                        } else {
                            document.write(response.html);
                        }
                        $('#hepsipay_payment_form_submit_btn').prop( "disabled", false );
                        $form.find('.processing-payment').hide();
                    }
                });
               return false;
            });
    ...
    $(document).ready(function(){
        var config = $('#hepsipay_payment_form').data('config');
        if(config) {
            hepsipayModule.run(config);
        }
    });
})(jQuery);